...
async function seed() {
    try {
        ...
        const existing = await (0, database_1.query)('SELECT * FROM users WHERE username = ?', ['admin']);
        if (existing.length > 0) {
            console.log('Admin user already exists. Skipping user seed.');
        }
        else {
            const salt = await bcryptjs_1.default.genSalt(10);
            const hash = await bcryptjs_1.default.hash('admin123', salt);
            await (0, database_1.query)('INSERT INTO users (username, email, password, role) VALUES (?, ?, ?, ?) RETURNING id', ['admin', 'admin@safariconnect.com', hash, 'admin']);
            console.log('Admin user created: admin / admin123');
        }
        const dests = await (0, database_1.query)('SELECT COUNT(*) as count FROM destinations');
        if (dests[0].count === '0' || dests[0].count === 0) {
            ...
            for (const [name, desc] of destData) {
                await (0, database_1.query)('INSERT INTO destinations (name, description) VALUES (?, ?)', [name, desc]);
            }
            console.log('Destinations seeded.');
        }
        const pkgs = await (0, database_1.query)('SELECT COUNT(*) as count FROM packages');
        if (pkgs[0].count === '0' || pkgs[0].count === 0) {
            ...
            for (const [name, dur, price, desc, activities] of pkgData) {
                await (0, database_1.query)('INSERT INTO packages (package_name, duration, price, description, activities_included) VALUES (?, ?, ?, ?, ?)', [name, dur, price, desc, activities]);
            }
            console.log('Packages seeded.');
        }
        const vehs = await (0, database_1.query)('SELECT COUNT(*) as count FROM vehicles');
        if (vehs[0].count === '0' || vehs[0].count === 0) {
            ...
            for (const [name, reg, cap, driver, avail] of vehData) {
                await (0, database_1.query)('INSERT INTO vehicles (vehicle_name, registration_number, capacity, driver_assigned, availability) VALUES (?, ?, ?, ?, ?)', [name, reg, cap, driver, avail]);
            }
            console.log('Vehicles seeded.');
        }
        const accs = await (0, database_1.query)('SELECT COUNT(*) as count FROM accommodations');
        if (accs[0].count === '0' || accs[0].count === 0) {
            ...
            for (const [name, loc, price, rooms, rating] of accData) {
                await (0, database_1.query)('INSERT INTO accommodations (accommodation_name, location, price_per_night, available_rooms, rating) VALUES (?, ?, ?, ?, ?)', [name, loc, price, rooms, rating]);
            }
